"""
Single Source of Truth for Database Schema - Version 4

Changes from v3:
- ADDED: photos.date_added (nullable TEXT) for recently-imported sorting
- ADDED: idx_date_added_recent index for flat import-date grid queries
"""

# Schema version for migrations
SCHEMA_VERSION = 4

# Photos table schema (v4: includes date_added column)
PHOTOS_TABLE_SCHEMA = """
...
# Hash cache table schema (retained from v3)
...
PHOTOS_INDICES = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_grid_newest ON photos(date_taken DESC, current_path ASC, id ASC)",
    "CREATE INDEX IF NOT EXISTS idx_grid_oldest ON photos(date_taken ASC, current_path ASC, id ASC)",
    "CREATE INDEX IF NOT EXISTS idx_undated_path ON photos(current_path ASC, id ASC) WHERE date_taken IS NULL",
    "CREATE INDEX IF NOT EXISTS idx_date_added_recent ON photos(date_added DESC, id DESC)",
]
...
def create_database_schema(cursor):
    """
    Create all tables and indices in the database (v4).
