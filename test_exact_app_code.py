#!/usr/bin/env python3
"""
Manual script: replicate app.py thumbnail generation for ICC checks.

Run directly: python3 test_exact_app_code.py
...
OUTPUT_FILE = os.path.join(
    ...
    "test_manual_app_replication.jpg",
)


def convert_to_rgb_properly(img):
    """Copy of convert_to_rgb_properly from app.py."""
    mode = img.mode
    if mode == "RGB":
        return img
...
def main():
    ...
    with Image.open(INPUT_FILE) as img:
        ...
        print(
            "5. ICC after convert_to_rgb_properly: "
            f"{len(icc_after_convert) if icc_after_convert else 0} bytes"
        )
        ...
        if icc_profile:
            ...
            print("10. Saving WITH icc_profile in save_kwargs")
        else:
            print("10. Saving WITHOUT icc_profile (icc_profile was None)")
