...
def resolve_port() -> int:
    """Port from PHOTOS_LIGHT_PORT (set by Electron), default 5001."""
    raw = os.environ.get("PHOTOS_LIGHT_PORT", str(DEFAULT_PORT))
    try:
        port = int(raw)
    except ValueError:
        return DEFAULT_PORT
...
def _open_browser_when_ready(url: str, port: int, timeout_seconds: float = 30.0) -> None:
