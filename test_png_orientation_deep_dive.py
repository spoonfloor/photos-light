#!/usr/bin/env python3
"""
Manual script: deep dive on PIL exif_transpose orientation tag behavior.

Run directly: python3 test_png_orientation_deep_dive.py
...
def main():
    ...
    result = subprocess.run(
        ["exiftool", "-Orientation", "-n", "-s3", test_copy],
        capture_output=True,
        text=True,
    )
    ...
    result = subprocess.run(
        ["sips", "-g", "pixelWidth", "-g", "pixelHeight", test_copy],
        capture_output=True,
        text=True,
    )
